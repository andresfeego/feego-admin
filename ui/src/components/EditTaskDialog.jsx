...
export default function EditTaskDialog({ open, onOpenChange, task, setTask, projects, sections, canEditProgress, saving, onSave, onArchive, onDelete }) {
  ...
  React.useEffect(()=>{setConfirmDelete(false);setDeleteError('')},[open,task.id])
  async function removeTask() {
    if(deleteLock.current)return
    deleteLock.current=true;setActionBusy(true);setDeleteError('')
    try { await onDelete() }
    catch(e){setDeleteError(e.message || 'No se pudo conectar para eliminar la tarea. Intenta de nuevo.')}
    finally{deleteLock.current=false;setActionBusy(false)}
  }
  ...
  return <Dialog.Root open={open} onOpenChange={v=>{if(!disabled)onOpenChange(v)}}><Dialog.Portal><Dialog.Overlay className="feego-overlay new-task-overlay" /><Dialog.Content className="feego-modal new-task-modal edit-task-modal">
    ...
    {confirmDelete && <div className="edit-task-confirm" role="alertdialog" aria-labelledby="delete-task-heading" aria-describedby="delete-task-description"><Icons.Trash2 size={25} /><h3 id="delete-task-heading">¿Eliminar tarea definitivamente?</h3><p id="delete-task-description">Se eliminará «{task.title}». Esta acción no se puede deshacer.</p>{deleteError && <p role="alert" className="new-task-error">{deleteError}</p>}<div><Button type="button" variant="outline" autoFocus disabled={disabled} onClick={()=>{setConfirmDelete(false);setDeleteError('')}}>Cancelar eliminación</Button><Button type="button" className="edit-task-confirm-delete" disabled={disabled} onClick={removeTask}><Icons.Trash2 size={16} />{actionBusy?'Eliminando…':'Sí, eliminar'}</Button></div></div>}
    ...
    <footer className="new-task-footer edit-task-footer" hidden={confirmDelete}><div className="edit-task-tools">{task.board!=='archived' && <Button type="button" variant="ghost" title="Archivar tarea" aria-label="Archivar tarea" disabled={disabled} onClick={archive}><Icons.Archive size={18} /></Button>}<Button type="button" variant="ghost" className="edit-task-delete" title="Eliminar tarea" aria-label="Eliminar tarea" disabled={disabled} onClick={()=>setConfirmDelete(true)}><Icons.Trash2 size={18} /></Button></div><div className="edit-task-save-actions"><Button type="button" variant="outline" disabled={disabled} onClick={()=>onOpenChange(false)}>Cancelar</Button><Button type="submit" disabled={disabled || !task.title?.trim()}><Icons.Check size={17} />{saving?'Guardando…':'Guardar cambios'}</Button></div></footer>
