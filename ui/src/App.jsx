...
import AppSidebar, { NavigationItems } from './components/AppSidebar.jsx'
  ...
  return (
    <>
      <div className="feego-mobile-header md:hidden fixed top-0 left-0 right-0 h-14 z-50 flex items-center gap-3 px-3 bg-gradient-to-r from-slate-950 to-slate-900 border-b border-white/10">
        <button onClick={() => setOpen(v => !v)} className="p-2 rounded-lg" aria-label={open ? "Cerrar menú" : "Abrir menú"} aria-expanded={open}>{open ? <X size={22} /> : <Menu size={22} />}</button>
...
function Layout() {
  ...
  React.useLayoutEffect(() => { contentRef.current?.scrollTo({ top: 0, left: 0 }) }, [pathname])
...
if (loading) {
  ...
  return (
    <div className="admin-shell">
      <MobileHeader />
      <div className="admin-layout flex md:pt-0 pt-14">
        <AppSidebar />
        <main ref={contentRef} className="admin-content flex-1 min-w-0 p-4 md:p-6 lg:p-8 2xl:p-12 max-w-full overflow-x-hidden">
          <Routes>
