// Remove only near-black pixels connected to the outside of Mako's logo.
// Keep the original asset intact; output a new PNG with transparency.
const sharp = require('sharp');
const path = require('node:path');
const root = path.resolve(__dirname, '..');
(async () => {
  const source = path.join(root, 'data/project-logos/project_25_1780333571013.webp');
  const output = path.join(root, 'data/project-logos/project_25_mako_transparent.png');
  ...
  function add(i) {
    if (seen[i]) return;
    const p=i*4;
    if (Math.max(data[p],data[p+1],data[p+2]) > 95) return;
  ...
  for(const i of queue) {
    ...
    const alpha=Math.max(0,Math.min(1,(brightness-25)/70));
  ...
  await sharp(output).flatten({background:'#ffffff'}).png().toFile('/tmp/mako-white-preview.png');
